...
import { useAddLikes } from '@/services/mutation/like/like'
import { useAddBookmarks } from '@/services/mutation/bookmark/bookmark'
import { useAddComments } from '@/services/mutation/comment/comment'
        ...
        {content.map((item, index) => {
            ...
            return (
              <p key={index} className="mb-4 text-gray-700 dark:text-gray-300 leading-relaxed">
                {item.children?.map((child, childIndex: number) => {
                  ...
                  if (child.bold) {
                    element = (
                      <strong key={childIndex} className="font-bold text-gray-900 dark:text-white">
                        {element}
                      </strong>
                    )
                  }
          ...
          if (item.type === 'header' && item.level) {
            ...
            const headingClasses = [
              ...
              item.level === 1 ? 'text-3xl' : item.level === 2 ? 'text-2xl' : 'text-xl'
            ].join(' ')
          ...
          if (item.type === 'list' && item.children) {
            ...
            const listClasses =
              item.format === 'ordered' ? 'list-decimal ml-6 mb-4' : 'list-disc ml-6 mb-4'

            return (
              <ListTag key={index} className={listClasses}>
                {item.children.map((listItem, listIndex: number) => (
          ...
          if (item.type === 'blockquote') {
            return (
              <blockquote
                key={index}
                className="border-l-4 border-primary pl-4 italic my-6 py-2 bg-gray-50 dark:bg-gray-800 rounded-r"
              >
                {item.children?.map((child) => child.text || '').join('')}
              </blockquote>
            )
...
function BlogPostViewContent() {
  ...
  const { data: postData, isLoading, error, refetch } = useGetPostById(documentId || '')
  const addLikeMutation = useAddLikes()
  const addBookmarkMutation = useAddBookmarks()
  ...
  const handleLike = () => {
    ...
    const payload = {
      data: {
        like_status: 'like', // Always send like, let the backend handle toggling
        blogposts: {
          connect: [
            {
              id: post.id,
    ...
    // Call the mutation and refetch to get updated data
    addLikeMutation.mutate(payload, {
      onSuccess: () => {
        refetch() // Refetch to get updated like data
      },
      onError: (error) => {
  ...
  const handleBookmark = () => {
    ...
    const payload = {
      data: {
        bookmark_status: 'saved', // Always send saved, let the backend handle toggling
        blogposts: {
          connect: [
            {
              id: post.id,
    ...
    addBookmarkMutation.mutate(payload, {
      onSuccess: () => {
        refetch() // Refetch to get updated bookmark data
      },
      onError: (error) => {
  ...
  const handleCommentSubmit = () => {
    ...
    addCommentMutation.mutate(payload, {
      onSuccess: () => {
        setCommentText('')
        refetch() // Refetch to get updated comment data
      },
      onError: (error) => {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      ...
      toast.className =
        'fixed bottom-4 right-4 bg-gray-800 text-white px-4 py-2 rounded-lg shadow-lg z-50 transition-opacity opacity-0'
  ...
  // Helper functions to get counts from API response
  const getLikeCount = () => {
    if (!post?.likes) return 0
    return post.likes.filter((like) => like.like_status === 'like').length
  }
  ...
  const isBookmarked = () => {
    ...
    return post.bookmarks.some((bookmark) => bookmark.bookmark_status === 'saved')
  }

  const isLiked = () => {
    if (!post?.likes) return false
    return post.likes.some((like) => like.like_status === 'like')
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
        <Card className="w-full max-w-md backdrop-blur-sm bg-white/80 dark:bg-gray-800/80 border-0 shadow-xl rounded-2xl overflow-hidden">
          <CardContent className="pt-8 pb-6 text-center">
            ...
            <Button
              asChild
              className="rounded-full px-6 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700"
            >
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        <div className="max-w-4xl mx-auto mb-8">
          <Button
            variant="ghost"
            asChild
            className="mb-6 group rounded-full pl-2 pr-4 transition-all duration-300 hover:bg-white/80 hover:shadow-sm backdrop-blur-sm"
          >
            <Link href="/dashboard">
          ...
          <div className="mb-8 p-6 rounded-2xl bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-white/20 dark:border-gray-700/30 shadow-sm">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
              ...
              {post.domains && post.domains.length > 0 && (
                <Badge
                  variant="secondary"
                  className="rounded-full px-3 py-1 bg-gradient-to-r from-blue-500 to-indigo-600"
                >
              ...
              {post.categories && post.categories.length > 0 && (
                <Badge
                  variant="outline"
                  className="rounded-full px-3 py-1 border-indigo-300 text-indigo-600 dark:text-indigo-400"
                >
          ...
          <div className="flex items-center gap-4 mb-8 p-4 rounded-2xl bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-white/20 dark:border-gray-700/30 shadow-sm">
            ...
            <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
              ...
              <span className="font-medium">{getCommentCount()} comments</span>
            </div>
